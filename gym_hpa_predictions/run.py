...
parser.add_argument('--test_path', default='f_a2c_env_redis_gym_goal_cost_k8s_False_totalSteps_25000_pred_lstm_dynamic_25000_steps.zip', help='Testing path, ex: logs/model/test.zip')
    ...
    for d in deployments:
        ...
        if method == 'ses':
           ...
           # save(pred.model) # run fitting once and save to file (takes a lot of time and the dataset is the same)
            ...
            x_test, y_test, scaler_test = pred.create_env_for_NN(pred.testset)
            predictions = pred.lstm_test(lstm_model, x_test, scaler_test)
        elif method == 'naive':
